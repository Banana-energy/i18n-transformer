...
import { localeWordPattern, transCode, transformTemplate } from './transform.js'
import { setConfig } from '../common/collect.js'
...
function findCommentExclude(path, ast) {
 ...
 */
function matchVueFileSpecialRule(path, id) {
...
export default function ({ id, code }, options) {
  const collection = []
  ...
  const visitor = {
      ...
      if (path.node.type === 'StringLiteral') {
        ...
        if (localePattern.test(val)) {
          ...
          if (res && res.length) {
            ...
            transform && transCode({ path, originValue: val, wordKeyMap, callee: i18nCallee })
          }
        }
      }
    },
    TemplateLiteral(path) {
      ...
      transform && transformTemplate({ path, callee: i18nCallee })
    }
  }
  ...
  const { code: newCode, map } = generator(
    ast,
    {
      sourceMaps: true
    },
